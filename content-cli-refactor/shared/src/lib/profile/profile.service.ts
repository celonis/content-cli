...
import {AuthenticationType, ClientAuthenticationMethod, Profile, ProfileType} from "./profile.interfaces";
...
export class ProfileService {
    ...
    public getAllFilesInDirectory(): string[] {
        ...
        try {
            ...
            logger.error(new FatalError(String(err)));
        }
        return fileNames;
    }
    ...
    private getBaseTeamUrl(teamUrl: string): string | null {
        if (!teamUrl) {
            return null;
        }
    ...
    private isProfileExpired(profile: Profile, buffer: number = 0): boolean {
        ...
        const now = new Date();
        const expirationTime = new Date((profile.expiresAt ?? 0) * 1000 - buffer);
