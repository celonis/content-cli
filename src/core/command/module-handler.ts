...
export class ModuleHandler {
   ...
   */
  public discoverAndRegisterModules(
    rootPath: string,
    ...
    try {
      const moduleFolders = fs.readdirSync(modulesDirPath, {
        withFileTypes: true,
      });
      ...
      for (const dirent of moduleFolders) {
        if (dirent.isDirectory()) {
          ...
          if (!potentialModuleJsPath) {
            ...
            try {
              logger.debug(
                `Found potential module definition: ${potentialModuleJsPath}`,
              );
              ...
              if (typeof ModuleClass === "function" && ModuleClass.prototype) {
                ...
                if (typeof moduleInstance.register === "function") {
                  ...
                  logger.warn(
                    `Module ${moduleFolderName} export does not have a 'register' method.`,
                  );
                }
              } else {
                logger.warn(
                  `Module ${moduleFolderName} export is not a class/constructor function.`,
                );
              }
            } catch (error: any) {
              if (error.code === "ENOENT") {
                ...
                logger.error(
                  `Error processing module in ${moduleFolderName}:`,
                  error,
                );
              }
            }
    ...
    logger.debug(
      `Module discovery complete. ${this.registeredModules.length} modules registered.`,
    );
  }
}
...
export class CommandConfig {
  ...
  public command(nameAndArgs: string, opts?: CommandOptions): CommandConfig {
    return new CommandConfig(
      this.cmd.command(nameAndArgs, opts),
      this.ctx,
  ...
  public action(handler: CommandHandler): void {
    this.cmd.action(async (): Promise<void> => {
      try {
        ...
        logger.error(
          `An unexpected error occured executing a command: ${error}`,
        );
      }
    });
  }
  ...
  private printBetaNoticeIfBetaCommand(): void {
    if ((this.cmd as any).isBeta) {
      logger.info(
        chalk.yellow(
          "This command is in beta and may change in future releases.",
        ),
      );
    }
  }
  ...
  private printBetaNoticeIfBetaOptions(): void {
    ...
    for (const option of this.cmd.options) {
      ...
      if (
        (option as any).isBeta &&
        Object.hasOwn(providedOptions, optionName)
      ) {
        logger.info(
          chalk.yellow(
            `The option '${option.long}' is in beta and may change in future releases.`,
          ),
        );
      }
    }
