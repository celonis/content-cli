...
export class Context {
  ...
  public get httpClient(): HttpClient {
    if (!this._httpClient) {
      throw new FatalError(
        "No profile provided. Please provide a profile or a team url and api token through env variables",
      );
    }
  ...
  private async loadGitProfile(
    gitProfileName: string | undefined,
  ): Promise<void> {
    if (!gitProfileName) {
    ...
    try {
      this.gitProfile =
        await this.gitProfileService.findProfile(gitProfileName);
