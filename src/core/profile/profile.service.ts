...
export class ProfileService {
  private profileContainerPath = path.resolve(
    homedir,
    ".celonis-content-cli-profiles",
  );
  private configContainer = path.resolve(
    this.profileContainerPath,
    "config.json",
  );
    ...
    return new Promise<Profile>((resolve, reject) => {
      try {
        if (!this.checkIfMissingProfile(profileName)) {
          ...
          this.refreshProfile(profile).then(() => resolve(profile));
  ...
  public storeProfile(profile: Profile): void {
    ...
    fs.writeFileSync(
      path.resolve(this.profileContainerPath, newProfileFileName),
  ...
  public async authorizeProfile(profile: Profile): Promise<void> {
    switch (profile.type) {
      case ProfileType.KEY:
        ...
        try {
          await this.tryKeyAuthentication(
            url,
          ...
          try {
            await this.tryKeyAuthentication(
              url,
              AuthenticationType.APPKEY,
              profile.apiToken,
            );
            profile.authenticationType = AuthenticationType.APPKEY;
          } catch (err) {
            logger.error(
              new FatalError("The provided team or api key is wrong."),
            );
            logger.error(err);
          }
        }
      ...
      case ProfileType.DEVICE_CODE:
        try {
          ...
          const deviceCodeHandle =
            await deviceCodeOAuthClient.deviceAuthorization({
              scope: deviceCodeScopes.join(" "),
            });
          logger.info(
            `Continue authorization here: ${deviceCodeHandle.verification_uri_complete}`,
          );
      ...
      case ProfileType.CLIENT_CREDENTIALS:
        ...
        try {
          ...
          const clientCredentialsOAuthClient =
            new clientCredentialsIssuer.Client({
              client_id: profile.clientId,
              client_secret: profile.clientSecret,
              token_endpoint_auth_method:
                ClientAuthenticationMethod.CLIENT_SECRET_BASIC,
            });
          const clientCredentialsTokenSet =
            await clientCredentialsOAuthClient.grant({
              grant_type: "client_credentials",
              scope: clientCredentialsScopes.join(" "),
            });
          profile.clientAuthenticationMethod =
            ClientAuthenticationMethod.CLIENT_SECRET_BASIC;
  ...
  public async refreshProfile(profile: Profile): Promise<void> {
    ...
    if (profile.type === ProfileType.DEVICE_CODE) {
      try {
        ...
        logger.error(
          new FatalError(
            "The profile cannot be refreshed. Please retry or recreate profile.",
          ),
        );
      }
    } else {
  ...
  private mapCelonisEnvProfile(): void {
    ...
    if (
      !celonisUrl.startsWith("http://") &&
      !celonisUrl.startsWith("https://")
    ) {
