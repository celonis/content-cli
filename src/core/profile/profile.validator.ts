...
export class ProfileValidator {
  public static async validateProfile(profile: Profile): Promise<any> {
    ...
    if (profile.type === ProfileType.KEY && profile.apiToken == null) {
      logger.error(
        new FatalError("The api token can not be empty for this profile type"),
      );
    }
    if (
      profile.type === ProfileType.CLIENT_CREDENTIALS &&
      (profile.clientId == null || profile.clientSecret == null)
    ) {
      logger.error(
        new FatalError(
          "The client id and secret can not be empty for this profile type",
        ),
      );
    }
