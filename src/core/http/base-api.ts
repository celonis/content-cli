    ...
    if (error && typeof error.status === "number") {
      ...
      switch (httpError.status) {
        case 401:
          ...
          throw new Error(
            `Unauthorized (401): ${httpError.message || "Authentication required"}`,
          ); // Placeholder
        case 403:
          throw new ForbiddenError(
            `Forbidden (403): ${httpError.message || "Access denied"}`,
          );
        case 404:
          throw new NotFoundError(
            `Not Found (404): ${httpError.message || "Resource not found"}`,
          );
        case 500:
        case 501:
        case 502:
        ...
        case 504:
          throw new ServerError(
            `Server Error (${httpError.status}): ${httpError.message || "Server issue"}`,
          );
        default:
