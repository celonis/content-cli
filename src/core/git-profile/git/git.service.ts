...
export class GitService {
  ...
  public async pushToBranch(sourceDir: string, branch: string): Promise<void> {
    ...
    try {
      ...
      await repoGit.addConfig(
        "user.name",
        this.gitProfile.username ?? "content-cli",
      );
      await repoGit.addConfig(
        "user.email",
        `${this.gitProfile.username ?? "cli"}@users.noreply.github.com`,
      );
  ...
  private async cleanupNonGitFiles(workingDir: string): Promise<void> {
    for (const entry of fs.readdirSync(workingDir)) {
      if (entry !== ".git") {
        fs.rmSync(path.join(workingDir, entry), {
          recursive: true,
          force: true,
        });
      }
  ...
  private async checkoutOrCreateLocalBranch(
    git: SimpleGit,
    branch: string,
  ): Promise<void> {
