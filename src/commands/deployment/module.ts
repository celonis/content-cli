...
class Module extends IModule {
  public register(context: Context, configurator: Configurator): void {
    ...
    deploymentCommand
      ...
      .requiredOption(
        ...
        "Identifier of the package to deploy",
      )
      .requiredOption(
      ...
      .requiredOption(
        "--targetId <targetId>",
        "Identifier of the target to deploy to",
      )
    ...
    listCommand
      ...
      .option(
        "--packageKey <packageKey>",
        "Filter deployment history by package key",
      )
      .option("--targetId <targetId>", "Filter deployment history by target ID")
      .option(
        "--deployableType <deployableType>",
        "Filter deployment history by deployable type",
      )
    ...
    listCommand
      ...
      .option(
        "--packageKey <packageKey>",
        "Identifier of the package to get the active deployment for",
      )
      .option(
        "--targetId <targetId>",
        "Identifier of the target to get the active deployment for",
      )
      .option(
        "--targetIds <targetIds>",
        "Comma-separated list of target IDs to get the active deployments for (only with packageKey)",
      )
      .option("--limit <limit>", "Limit the number of results returned")
      .option("--offset <offset>", "Offset for pagination")
      .option("--json", "Return the response as a JSON file")
    ...
    listCommand
      ...
      .beta()
      .description(
        "List all targets for a given deployable type and package key",
      )
      .requiredOption(
        "--deployableType <deployableType>",
        "The type of the deployable",
      ...
      .requiredOption(
        "--packageKey <packageKey>",
        "Identifier of the package to list targets for",
      )
      .option("--json", "Return the response as a JSON file")
  ...
  private async createDeployment(
    ...
    await new DeploymentService(context).createDeployment(
      options.packageKey,
      options.packageVersion,
      options.deployableType,
      options.targetId,
      options.json,
    );
  }

  private async listDeploymentHistory(
    context: Context,
    command: Command,
    options: OptionValues,
  ): Promise<void> {
  ...
  private async listActiveDeployments(
    context: Context,
    command: Command,
    options: OptionValues,
  ): Promise<void> {
    if (options.packageKey && options.targetId) {
      throw new Error(
        "Please provide either --packageKey or --targetId, not both.",
      );
    }

    if (options.packageKey) {
      const targetIds: string[] = options.targetIds
        ? options.targetIds.split(",")
        : [];
      await new DeploymentService(context).getActiveDeploymentsForPackage(
        options.packageKey,
        options.json,
        targetIds,
        options.limit,
      ...
      await new DeploymentService(context).getActiveDeploymentForTarget(
        options.targetId,
        options.json,
      );
    } else {
  ...
  private async listDeployables(
    context: Context,
    command: Command,
    options: OptionValues,
  ): Promise<void> {
    await new DeploymentService(context).getDeployables(
      options.json,
      options.flavor,
    );
  }
  ...
  private async listTargets(
    ...
    await new DeploymentService(context).getTargets(
      options.json,
      options.deployableType,
      options.packageKey,
    );
