...
class Module extends IModule {
    ...
    public register(context: Context, configurator: Configurator): void {
        ...
        listCommand.command("active")
            ...
            .description("You can use the command to list the active deployment(s) for a specific target or for a specific package.")
            .description("The targetIds filter is available only for getting the active deployments for a given package.")
            .description("Listing active deployments for a package returns paginated results. Use the limit and offset parameters to navigate through the results.")
    ...
    private async listDeploymentHistory(context: Context, command: Command, options: OptionValues): Promise<void> {
        await new DeploymentService(context).getDeployments(options.json, options.packageKey, options.targetId, options.deployableType, options.status, options.createdBy);
    }
