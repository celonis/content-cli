...
export class WidgetManagerFactory {
  ...
  public fetchManifest(): Manifest {
    if (fs.existsSync(path.resolve(process.cwd(), "manifest.yaml"))) {
      return parse(
        fs.readFileSync(path.resolve(process.cwd(), "manifest.yaml"), {
          encoding: "utf-8",
        }),
      );
    }
    ...
    if (fs.existsSync(path.resolve(process.cwd(), "manifest.yml"))) {
      return parse(
        fs.readFileSync(path.resolve(process.cwd(), "manifest.yml"), {
          encoding: "utf-8",
        }),
      );
    }
  ...
  public validateManifest(manifest: Manifest): void {
    ...
    if (
      ...
      !fs.existsSync(
        path.resolve(process.cwd(), "assets", "widgets", manifest.key),
      )
    ) {
      logger.error(
        new FatalError(
          "Assets directory does not exist. Assets should live under 'assets/widgets/" +
            manifest.key +
            "'.",
        ),
      );
