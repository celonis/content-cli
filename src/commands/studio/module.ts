...
class Module extends IModule {
  public register(context: Context, configurator: Configurator): void {
    ...
    pushCommand
      ...
      .requiredOption(
        "--package <packageKey>",
        "Key of the package you want to push asset to",
      )
    ...
    pushCommand
      ...
      .requiredOption(
        "--package <packageKey>",
        "Key of the package you want to push assets to",
      )
    ...
    pushCommand
      ...
      .requiredOption(
        "--spaceKey <spaceKey>",
        "The key of the destination space",
      )
    ...
    pushCommand
      ...
      .option(
        "--userSpecific",
        "Upload widget only for the user in the provided api token",
      )
      .option(
        "--packageManager",
        "Upload widget to package manager (deprecated)",
      ) // Deprecated
  ...
  private async listAssets(
    ...
    await new AssetCommandService(context).listAssets(
      options.json,
      options.assetType,
    );
  }

  private async pullAsset(
    context: Context,
    command: Command,
    options: OptionValues,
  ): Promise<void> {
  ...
  private async pushAsset(
    ...
    await new AssetCommandService(context).pushAsset(
      options.file,
      options.package,
    );
  }

  private async pushAssets(
    context: Context,
    command: Command,
    options: OptionValues,
  ): Promise<void> {
  ...
  private async pushWidget(
    context: Context,
    command: Command,
    options: OptionValues,
  ): Promise<void> {
    await new WidgetCommand(context).pushWidget(
      !!options.tenantIndependent,
