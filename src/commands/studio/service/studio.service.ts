...
export class StudioService {
    ...
    public async getExportPackagesWithStudioData(packagesToExport: PackageExportTransport[], withDependencies: boolean): Promise<PackageExportTransport[]> {
        const studioPackagesWithDataModels = await this.packageApi.findAllPackagesWithVariableAssignments(PackageManagerVariableType.DATA_MODEL);

        packagesToExport = this.setSpaceIdForStudioPackages(packagesToExport, studioPackagesWithDataModels);

        if (withDependencies) {
            const dataModelDetailsByNode = await this.dataModelService.getDataModelDetailsForPackages(studioPackagesWithDataModels);
            packagesToExport = this.setDataModelsForStudioPackages(packagesToExport, studioPackagesWithDataModels, dataModelDetailsByNode);
        }
    ...
    public async getStudioPackageManifests(studioPackageKeys: string[]): Promise<StudioPackageManifest[]> {
        return Promise.all(studioPackageKeys.map(async packageKey => {
            const node = await this.nodeApi.findOneByKeyAndRootNodeKey(packageKey, packageKey);
            const nodeSpace: SpaceTransport = await this.spaceApi.findOne(node.spaceId);
            const variableAssignments = await this.variablesApi.getRuntimeVariableValues(packageKey, BatchExportImportConstants.APP_MODE_VIEWER);
    ...
    public async processImportedPackages(configs: AdmZip, existingStudioPackages: ContentNodeTransport[], studioManifests: StudioPackageManifest[]): Promise<void> {
        ...
        for (const  manifest of studioManifests) {
            ...
            if (existingPackage) {
                await this.packageApi.movePackageToSpace(existingPackage.id, manifest.space.id);
            }
    ...
    private async findDesiredSpaceIdForPackage(studioPackageManifest: StudioPackageManifest): Promise<string> {
        ...
        const spaceTransport = await this.spaceService.createSpace(studioPackageManifest.space.name, studioPackageManifest.space.iconReference);
    ...
    private async assignRuntimeVariables(manifest: StudioPackageManifest): Promise<void> {
        if (manifest.runtimeVariableAssignments.length) {
            await this.variableService.assignVariableValues(manifest.packageKey, manifest.runtimeVariableAssignments);
        }
    }
