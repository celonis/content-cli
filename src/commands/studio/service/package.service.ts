...
export class PackageService {
  protected readonly fileDownloadedMessage =
  ...
  public async findAndExportListOfAllPackages(
    ...
    let nodesListToExport: BatchExportNodeTransport[] =
      await this.packageApi.findAllPackages();
    if (packageKeys.length > 0) {
      nodesListToExport = nodesListToExport.filter(node =>
        packageKeys.includes(node.rootNodeKey),
      );
    }
    ...
    if (includeDependencies) {
      ...
      const unPublishedNodes = nodesListToExport.filter(
        node => !node.activatedDraftId,
      );
      let publishedNodes = nodesListToExport.filter(
        node => node.activatedDraftId,
      );
      publishedNodes = await this.getNodesWithActiveVersion(publishedNodes);
      nodesListToExport = [...publishedNodes, ...unPublishedNodes];

      const packageWithDataModelVariableAssignments =
        await this.variableService.getVariableAssignmentsForNodes(
          PackageManagerVariableType.DATA_MODEL,
        );
      const dataModelDetailsByNode =
        await this.dataModelService.getDataModelDetailsForPackages(
          packageWithDataModelVariableAssignments,
        );
      ...
      nodesListToExport.forEach(node =>
        draftIdByNodeId.set(node.workingDraftId, node.id),
      );

      const dependenciesByPackageIds =
        await this.getPackagesWithDependencies(draftIdByNodeId);

      nodesListToExport = nodesListToExport.map(nodeToExport => {
        nodeToExport.dependencies =
          dependenciesByPackageIds[nodeToExport.workingDraftId] ?? [];
        return nodeToExport;
      });
    }
  ...
  public async getNodesWithActiveVersion(
    nodes: BatchExportNodeTransport[],
  ): Promise<BatchExportNodeTransport[]> {
    const activeVersionsOfPackage =
      await this.packageApi.findActiveVersionByIds(nodes.map(node => node.id));

    nodes.forEach(node => {
      node.version = activeVersionsOfPackage.find(
        packageVersion => packageVersion.id === node.id,
      );
    });
  ...
  public async getPackagesWithDependencies(
    draftIdByNodeId: Map<string, string>,
  ): Promise<Map<string, PackageDependencyTransport[]>> {
    return await this.packageDependenciesApi.findPackageDependenciesByIds(
      draftIdByNodeId,
    );
  }

  private exportListOfPackages(
    nodes: BatchExportNodeTransport[],
    fieldsToInclude: string[],
  ): void {
    const filename = uuidv4() + ".json";
    fileService.writeToFileWithGivenName(
      JSON.stringify(nodes, fieldsToInclude),
      filename,
    );
