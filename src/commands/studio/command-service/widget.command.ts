...
export class WidgetCommand {
  ...
  public async pushWidget(
    ...
    await this.widgetManagerFactory
      .createManager(tenantIndependent, userSpecific)
      .push();
  ...
  private async pushToAwsIfAuthorized(): Promise<void> {
    if (
      process.env.AWS_ACCESS_KEY_ID_CDN &&
      process.env.AWS_SECRET_ACCESS_KEY_CDN
    ) {
      try {
        ...
        logger.error(
          new GracefulError(error.stderr?.toString() || error.message),
        );
      }
    }
