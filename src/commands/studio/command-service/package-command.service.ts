...
export class PackageCommandService {
    ...
    public async pullPackage(key: string, store: boolean, newKey: string, draft: boolean): Promise<void> {
        await this.packageManagerFactory.createPullManager(key, store, newKey, draft).pullFile();
    }

    public async pushPackage(spaceKey: string, fileName: string, newKey: string, overwrite: boolean): Promise<void> {
