...
export class PackageCommandService {
  ...
  public async pullPackage(
    ...
    await this.packageManagerFactory
      .createPullManager(key, store, newKey, draft)
      .pullFile();
  }
  ...
  public async pushPackage(
    ...
    await this.packageManagerFactory
      .createPushManager(spaceKey, fileName, newKey, overwrite)
      .push();
  }

  public async pushPackages(spaceKey: string): Promise<void> {
    const packageManagers =
      this.packageManagerFactory.createPushManagers(spaceKey);
  ...
  public async listPackages(
    ...
    if (jsonResponse) {
      await this.packageService.findAndExportListOfAllPackages(
        includeDependencies,
        packageKeys ?? [],
      );
    } else {
