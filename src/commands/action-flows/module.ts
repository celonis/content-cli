...
class Module extends IModule {
  public register(context: Context, configurator: Configurator): void {
    ...
    analyzeCommand
      ...
      .requiredOption(
        "--packageId <packageId>",
        "ID of the package from which you want to export Action Flows",
      )
      .option(
        "-o, --outputToJsonFile",
        "Output the analyze result in a JSON file",
      )
    ...
    exportCommand
      .command("action-flows")
      .description(
        "Command to export all Action Flows in a package with their objects and dependencies",
      )
      .requiredOption(
        "--packageId <packageId>",
        "ID of the package from which you want to export Action Flows",
    ...
    importCommand
      .command("action-flows")
      .description(
        "Command to import all Action Flows in a package with their objects and dependencies",
      )
      .requiredOption(
        "--packageId <packageId>",
        "ID of the package to which you want to export Action Flows",
      )
      .requiredOption(
        "-f, --file <file>",
        "Exported Action Flows file (relative path)",
      )
      .requiredOption(
        "-d, --dryRun <dryRun>",
        "Execute the import on dry run mode",
      )
      .option(
        "-o, --outputToJsonFile",
        "Output the import result in a JSON file",
      )
    ...
    pullCommand
      ...
      .requiredOption(
        "--projectId <projectId>",
        "Id of the project you want to pull",
      )
    ...
    pushCommand
      ...
      .requiredOption(
        "--projectId <projectId>",
        "Id of the project you want to push",
      )
      .requiredOption("-f, --file <file>", "The file you want to push")
      .action(this.pushSkill);
  }

  private async analyzeActionFlows(
    context: Context,
    command: Command,
    options: OptionValues,
  ): Promise<void> {
    await new ActionFlowCommandService(context).analyzeActionFlows(
      options.packageId,
      options.outputToJsonFile,
    );
  }

  private async exportActionFlows(
    context: Context,
    command: Command,
    options: OptionValues,
  ): Promise<void> {
    await new ActionFlowCommandService(context).exportActionFlows(
      options.packageId,
      options.file,
    );
  }

  private async importActionFlows(
    context: Context,
    command: Command,
    options: OptionValues,
  ): Promise<void> {
    await new ActionFlowCommandService(context).importActionFlows(
      options.packageId,
      options.file,
      options.dryRun,
      options.outputToJsonFile,
  ...
  private async pullSkill(
    ...
    await new SkillCommandService(context).pullSkill(
      options.profile,
      options.projectId,
      options.skillId,
    );
  ...
  private async pushSkill(
    ...
    await new SkillCommandService(context).pushSkill(
      options.profile,
      options.projectId,
      options.file,
    );
