...
export class ActionFlowService {
  ...
  public async exportActionFlows(
    packageId: string,
    metadataFilePath: string,
  ): Promise<void> {
    const exportedActionFlowsData =
      await this.actionFlowApi.exportRawAssets(packageId);
  ...
  public async analyzeActionFlows(
    packageId: string,
    outputToJsonFile: boolean,
  ): Promise<void> {
    const actionFlowsMetadata =
      await this.actionFlowApi.analyzeAssets(packageId);
    const actionFlowsMetadataString = JSON.stringify(
      actionFlowsMetadata,
      null,
      4,
    );
    ...
    if (outputToJsonFile) {
      ...
      logger.info(
        "Action flows analyze metadata: \n" + actionFlowsMetadataString,
      );
    }
  }

  public async importActionFlows(
    packageId: string,
    filePath: string,
    dryRun: boolean,
    outputToJsonFile: boolean,
    ...
    const eventLog = await this.actionFlowApi.importAssets(
      packageId,
      actionFlowsZip,
      dryRun,
    );
    ...
    if (outputToJsonFile) {
      const eventLogFileName =
        "action-flows_import_event_log_" + uuidv4() + ".json";
  ...
  private createBodyForImport(fileName: string): FormData {
    ...
    formData.append("file", fs.createReadStream(fileName, { encoding: null }), {
      filename: fileName,
    });
