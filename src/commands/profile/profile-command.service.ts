...
export class ProfileCommandService {
  ...
  public async createProfile(setAsDefault: boolean): Promise<void> {
    ...
    try {
      ...
      profile.team = await questions.ask(
        "Your team (please provide the full url): ",
      );
      const type = await questions.ask(
        "Profile type: OAuth Device Code (1), OAuth Client Credentials (2) or Application Key / API Key (3): ",
      );
      switch (type) {
        case "1":
