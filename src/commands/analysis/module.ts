...
class Module extends IModule {
  public register(context: Context, configurator: Configurator): void {
    ...
    pullCommand
      ...
      .option(
        "--type <type>",
        "Pull shared/all Analysis Bookmarks, else by default get user bookmarks",
      )
    ...
    pushCommand
      ...
      .requiredOption(
        "--id <id>",
        "Id of the Analysis to which you want to push the analysis bookmarks",
      )
      .requiredOption("-f, --file <file>", "The file you want to push")
      .action(this.pushAnalysisBookmarks);
  }

  private async pullAnalysisBookmarks(
    context: Context,
    command: Command,
    options: OptionValues,
  ): Promise<void> {
    await new AnalysisBookmarksCommandService(context).pullAnalysisBookmarks(
      options.id,
      options.type,
    );
  }

  private async pushAnalysisBookmarks(
    context: Context,
    command: Command,
    options: OptionValues,
  ): Promise<void> {
    await new AnalysisBookmarksCommandService(context).pushAnalysisBookmarks(
      options.id,
      options.file,
    );
  }
