...
export class VariableService {
  ...
  constructor(context: Context) {
    ...
    this.variableAssignmentCandidatesApi = new VariableAssignmentCandidatesApi(
      context,
    );
  ...
  public async listVariables(
    ...
    const variableManifests = await this.getVersionedVariablesByKeyVersionPairs(
      keysByVersion,
      keysByVersionFile,
    );
  ...
  public async listCandidateAssignments(
    type: string,
    params: string,
  ): Promise<void> {
    const parsedParams = this.parseParams(params);
    const assignments =
      await this.variableAssignmentCandidatesApi.getCandidateAssignments(
        type,
        parsedParams,
      );
  ...
  public async findAndExportCandidateAssignments(
    type: string,
    params: string,
  ): Promise<void> {
    const parsedParams = this.parseParams(params);
  ...
  private async getVersionedVariablesByKeyVersionPairs(
    ...
    const variablesExportRequest: PackageKeyAndVersionPair[] =
      await this.buildKeyVersionPairs(keysByVersion, keysByVersionFile);

    const variableManifests =
      await this.batchImportExportApi.findVariablesWithValuesByPackageKeysAndVersion(
        variablesExportRequest,
      );
  ...
  private async buildKeyVersionPairs(
    ...
    if (keysByVersion.length !== 0) {
      ...
      variablesExportRequest =
        await fileService.readFileToJson(keysByVersionFile);
    } else {
  ...
  private buildKeyAndVersionPairsFromArrayInput(
    keysByVersion: string[],
  ): PackageKeyAndVersionPair[] {
