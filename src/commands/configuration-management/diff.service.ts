...
} from "./interfaces/diff-package.interfaces";
...
export class DiffService {
  ...
  private async hasChanges(file: string, jsonResponse: boolean): Promise<void> {
    ...
    if (jsonResponse) {
      ...
      logger.info(
        this.buildStringResponseForPackageDiffMetadataList(
          returnedHasChangesData,
        ),
      );
    }
  }

  private async diffPackagesAndReturnDiff(
    file: string,
    jsonResponse: boolean,
  ): Promise<void> {
    const packages = new AdmZip(file);
    ...
    if (jsonResponse) {
      ...
      logger.info(
        this.buildStringResponseForPackageDiffs(returnedHasChangesData),
      );
    }
  }
  ...
  private exportListOfPackageDiffs(packageDiffs: PackageDiffTransport[]): void {
    ...
    fileService.writeToFileWithGivenName(
      JSON.stringify(packageDiffs),
      filename,
    );
    logger.info(FileService.fileDownloadedMessage + filename);
  }

  private exportListOfPackageDiffMetadata(
    packageDiffMetadata: PackageDiffMetadata[],
  ): void {
    const filename = uuidv4() + ".json";
    fileService.writeToFileWithGivenName(
  ...
  private buildStringResponseForPackageDiffs(
    packageDiffs: PackageDiffTransport[],
  ): string {
    return "\n" + JSON.stringify(packageDiffs, null, 2);
  }

  private buildStringResponseForPackageDiffMetadataList(
    packageDiffMetadata: PackageDiffMetadata[],
  ): string {
