...
export class BatchImportExportService {
    ...
    public async batchExportPackages(
        ...
        if (packageKeys) {
            ...
            exportedPackagesData = await this.batchImportExportApi.exportPackagesByVersions(
        ...
        const studioPackageKeys = manifest
            .filter(packageManifest => packageManifest.flavor === BatchExportImportConstants.STUDIO)
        ...
        exportedPackagesZip.getEntries().forEach(entry => {
            if (entry.name.endsWith(BatchExportImportConstants.ZIP_EXTENSION)) {
                ...
                if (studioPackageKeys.includes(packageKey)) {
                    const updatedPackage = this.studioService.processPackageForExport(entry, exportedVariables);
    ...
    public async findAndExportListOfActivePackagesByVariableValue(
        ...
        let packagesToExport = await this.batchImportExportApi.findActivePackagesByVariableValue(
            flavors,
            variableValue,
            variableType
        );
    ...
    public async listActivePackagesByVariableValue(
        ...
        const packagesByVariableValue = await this.batchImportExportApi.findActivePackagesByVariableValue(
            flavors,
            variableValue,
            variableType
        );
