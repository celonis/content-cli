...
export class StudioService {
  ...
  public async getExportPackagesWithStudioData(
    ...
    const studioPackagesWithDataModels =
      await this.studioPackageApi.findAllPackagesWithVariableAssignments(
    ...
    if (withDependencies) {
      const dataModelDetailsByNode =
        await this.studioDataModelService.getDataModelDetailsForPackages(
          studioPackagesWithDataModels,
        );
  ...
  public async getStudioPackageManifests(
    studioPackageKeys: string[],
  ): Promise<StudioPackageManifest[]> {
    return Promise.all(
      studioPackageKeys.map(async packageKey => {
        const node = await this.studioNodeApi.findOneByKeyAndRootNodeKey(
          packageKey,
          packageKey,
        );
        const nodeSpace: SpaceTransport = await this.studioSpaceApi.findOne(
          node.spaceId,
        );
        const variableAssignments =
          await this.studioVariablesApi.getRuntimeVariableValues(
            packageKey,
  ...
  public processPackageForExport(
    exportedPackage: IZipEntry,
    ...
    this.fixConnectionVariablesForRootNodeFiles(
      packageZip,
  ...
  public async processImportedPackages(
    ...
    for (const manifest of studioManifests) {
      ...
      if (existingPackage) {
        await this.studioPackageApi.movePackageToSpace(
          existingPackage.id,
          manifest.space.id,
        );
      }
  ...
  private setSpaceIdForStudioPackages(
    ...
    const studioPackageByKey = new Map<
      string,
      PackageWithVariableAssignments
    >();
  ...
  private setDataModelsForStudioPackages(
    ...
    const studioPackageByKey = new Map<
      ...
      PackageWithVariableAssignments
    >();
    studioPackageWithDataModels.forEach(pkg =>
      studioPackageByKey.set(pkg.key, pkg),
    );
  ...
  private fixConnectionVariable(
    variable: VariableExportTransport,
  ): VariableExportTransport {
      ...
      .filter(
        entry =>
          entry.entryName.startsWith(
            BatchExportImportConstants.NODES_FOLDER_NAME,
          ) &&
  ...
  private fixConnectionVariablesForRootNodeFiles(
    ...
    const exportedNode: NodeExportTransport = parse(
      packageEntry.getData().toString(),
    );
  ...
  private getConnectionVariablesByKeyForPackage(
    ...
    const packageVariables = variables.find(
      exportedVariable =>
        exportedVariable.packageKey === packageKey &&
        exportedVariable.version === packageVersion,
    );

    if (packageVariables && packageVariables.variables.length) {
      packageVariables.variables
        .filter(
          variable => variable.type === PackageManagerVariableType.CONNECTION,
        )
  ...
  public async mapSpaces(
    exportedFiles: AdmZip,
    studioManifests: StudioPackageManifest[],
  ): Promise<AdmZip> {
    if (studioManifests == null) {
    ...
    for (const file of exportedFiles.getEntries()) {
      if (file.name.endsWith(BatchExportImportConstants.ZIP_EXTENSION)) {
        ...
        if (this.isStudioPackage(studioManifests, packageKey)) {
          const studioManifest = studioManifests.find(
            manifest => manifest.packageKey === packageKey,
          );

          if (studioManifest) {
            const spaceId =
              await this.findDesiredSpaceIdForPackage(studioManifest);
            studioManifest.space.id = spaceId;

            const packageZip = new AdmZip(file.getData());
            packageZip.getEntries().forEach(nodeFile => {
              if (
                nodeFile.entryName.endsWith(
                  BatchExportImportConstants.JSON_EXTENSION,
                )
              ) {
  ...
  private async findDesiredSpaceIdForPackage(
    studioPackageManifest: StudioPackageManifest,
  ): Promise<string> {
    ...
    if (studioPackageManifest.space.id) {
      const targetSpace = allSpaces.find(
        space => space.id === studioPackageManifest.space.id,
      );
      if (!targetSpace) {
    ...
    const targetSpaceByName = allSpaces.find(
      space => space.name === studioPackageManifest.space.name,
    );
  ...
  private async assignRuntimeVariables(
