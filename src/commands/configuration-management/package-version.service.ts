...
export class PackageVersionService {
  ...
  public async findPackageVersion(
    packageKey: string,
    version: string,
    jsonResponse: boolean,
  ): Promise<void> {
    const packageVersionTransport: PackageVersionTransport =
      await this.packageVersionApi.findOne(packageKey, version);
    if (jsonResponse) {
      const filename = uuidv4() + ".json";
      fileService.writeToFileWithGivenName(
        JSON.stringify(packageVersionTransport, null, 2),
        filename,
      );
  ...
  private printPackageVersionTransport(
    packageVersionTransport: PackageVersionTransport,
  ): void {
    ...
    logger.info(
      `Change Date: ${new Date(packageVersionTransport.changeDate).toISOString()}`,
    );
    logger.info(
      `Publish Date: ${new Date(packageVersionTransport.publishDate).toISOString()}`,
    );
