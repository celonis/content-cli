...
class Module extends IModule {
  public register(context: Context, configurator: Configurator): void {
    ...
    configCommand
      ...
      .option(
        "--flavors <flavors...>",
        "Lists only active packages of the given flavors",
      )
      .option("--withDependencies", "Include dependencies", "")
      .option("--packageKeys <packageKeys...>", "Lists only given package keys")
      .option(
        "--variableValue <variableValue>",
        "Variable value for filtering packages by.",
      )
      .option(
        "--variableType <variableType>",
        "Variable type for filtering packages by.",
      )
    ...
    configCommand
      ...
      .option(
        ...
        "Keys of packages to export. Exports the latest deployed version only",
      )
      .option(
        "--keysByVersion <keysByVersion...>",
        "Keys of packages to export by version",
      )
      .option("--withDependencies", "Include variables and dependencies", "")
      .option("--unzip", "Unzip the exported file", "")
      .option(
        "--gitProfile <gitProfile>",
        "Git profile which you want to use for the Git operations",
      )
      .option(
        "--gitBranch <gitBranch>",
        "Git branch in which you want to push the exported file",
      )
      .action(this.batchExportPackages);

    const metadataCommand = configCommand
      .command("metadata")
    ...
    metadataCommand
      ...
      .requiredOption(
        "--packageKeys <packageKeys...>",
        "Keys of packages to find the metadata of",
      )
      .option("--json", "Return response as json type", "")
    ...
    configCommand
      ...
      .option(
        "--gitBranch <gitBranch>",
        "Git branch from which you want to pull the exported file and import",
      )
      .option("-f, --file <file>", "Exported packages file (relative path)")
      .option(
        "-d, --directory <directory>",
        "Exported packages directory (relative path)",
      )
    ...
    configCommand
      ...
      .option(
        "--hasChanges",
        "Flag to return only the information if the package has changes without the actual changes",
      )
      .option("--json", "Return the response as a JSON file")
      .requiredOption(
        "-f, --file <file>",
        "Exported packages file (relative or absolute path)",
      )
      .action(this.diffPackages);

    const configVersionCommand = configCommand
      .command("versions")
    ...
    configVersionCommand
      ...
      .requiredOption(
        ...
        'Version of the package. Can be a specific version or "LATEST" keyword, which will return the most recently created version.',
      )
      .option("--json", "Return the response as a JSON file")
      .action(this.getPackageVersion);

    const variablesCommand = configCommand
      .command("variables")
    ...
    variablesCommand
      ...
      .option(
        "--keysByVersion <keysByVersion...>",
        "Mapping of package keys and versions",
        "",
      )
      .option(
        "--keysByVersionFile <keysByVersionFile>",
        "Package keys by version mappings file path.",
        "",
      )
      .action(this.listVariables);

    const nodesCommand = configCommand
      .command("nodes")
    ...
    nodesCommand
      ...
      .option(
        "--withConfiguration",
        "Include node configuration in the response",
        false,
      )
      .option("--json", "Return the response as a JSON file")
    ...
    nodesCommand
      ...
      .requiredOption(
        ...
        "Compare version of the node",
      )
      .option("--json", "Return the response as a JSON file")
      .action(this.diffNode);

    const nodeDependenciesCommand = nodesCommand
      .command("dependencies")
      .description("Commands related to node dependencies");

    nodeDependenciesCommand
      .command("list")
      ...
      .option(
        "--packageVersion <packageVersion>",
        "Version of the package. If not sent, the staging state of the package will be used.",
      )
      .option("--json", "Return the response as a JSON file")
  ...
  private async listActivePackages(
    context: Context,
    command: Command,
    options: OptionValues,
  ): Promise<void> {
    await new ConfigCommandService(context).listActivePackages(
      options.json,
      options.flavors,
  ...
  private async batchExportPackages(
    context: Context,
    command: Command,
    options: OptionValues,
  ): Promise<void> {
    if (
      (options.packageKeys && options.keysByVersion) ||
      (!options.packageKeys && !options.keysByVersion)
    ) {
      throw new Error(
    ...
    await new ConfigCommandService(context).batchExportPackages(
      options.packageKeys,
  ...
  private async batchExportPackagesMetadata(
    context: Context,
    command: Command,
    options: OptionValues,
  ): Promise<void> {
    await new ConfigCommandService(context).batchExportPackagesMetadata(
      options.packageKeys,
      options.json,
    );
  }
  ...
  private async getPackageVersion(
    ...
    await new PackageVersionCommandService(context).getPackageVersion(
      options.packageKey,
  ...
  private async batchImportPackages(
    context: Context,
    command: Command,
    options: OptionValues,
  ): Promise<void> {
    ...
    await new ConfigCommandService(context).batchImportPackages(
      options.file,
      options.directory,
      options.overwrite,
      options.gitBranch,
  ...
  private async diffPackages(
    ...
    await new ConfigCommandService(context).diffPackages(
      options.file,
      options.hasChanges,
      options.json,
    );
  ...
  private async listAssignments(
    ...
    await new VariableCommandService(context).listAssignments(
      options.type,
      options.json,
      options.params,
    );
  ...
  private async findNode(
    ...
    await new NodeService(context).findNode(
      options.packageKey,
      options.nodeKey,
  ...
  private async listNodes(
    ...
    await new NodeService(context).listNodes(
      options.packageKey,
      options.packageVersion,
      options.limit,
      options.offset,
  ...
  private async diffNode(
    context: Context,
    command: Command,
    options: OptionValues,
  ): Promise<void> {
    await new NodeDiffService(context).diff(
      options.packageKey,
      options.nodeKey,
      options.baseVersion,
  ...
  private async listNodeDependencies(
    context: Context,
    command: Command,
    options: OptionValues,
  ): Promise<void> {
    ...
    await new NodeDependencyService(context).listNodeDependencies(
      options.packageKey,
      options.nodeKey,
      options.packageVersion,
      options.json,
