...
export class NodeDependencyApi {
  ...
  public async findAllStaging(
    packageKey: string,
    nodeKey: string,
  ): Promise<NodeDependencyTransport[]> {
    return this.httpClient()
      .get(
        `/pacman/api/core/staging/packages/${packageKey}/nodes/${nodeKey}/dependencies`,
      )
      .catch(e => {
        throw new FatalError(
          `Problem finding dependencies for node ${nodeKey} in package ${packageKey}: ${e}`,
        );
      });
  }
  ...
  public async findAllByVersion(
    ...
    return this.httpClient()
      .get(
        `/pacman/api/core/packages/${packageKey}/nodes/${nodeKey}/dependencies?${queryParams.toString()}`,
      )
      .catch(e => {
        throw new FatalError(
