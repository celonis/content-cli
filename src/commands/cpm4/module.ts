...
class Module extends IModule {
  public register(context: Context, configurator: Configurator): void {
    ...
    pushCommand
      ...
      .option(
        "-a, --pushAnalysis",
        "Specify this option if you want to push analysis from the CTP file",
      )
      .option(
        "-d, --pushDataModels",
        "Specify this option if you want to push data models from the CTP file",
      )
      .option(
      ...
      .requiredOption(
        "--password <password>",
        "The password used for extracting the .ctp file",
      )
      .action(this.pushCTPFile);
  }

  private async pushCTPFile(
    context: Context,
    command: Command,
