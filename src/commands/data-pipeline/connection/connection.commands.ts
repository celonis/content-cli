...
export class ConnectionCommands {
  public register(context: Context, configurator: Configurator): void {
    ...
    getCommand
      ...
      .requiredOption(
        ...
        "Id of the data pool you want to update",
      )
      .requiredOption(
        "--connectionId <connectionId>",
        "Id of the connection you want to update",
      )
  ...
  private async getCommandProperties(
    context: Context,
    command: Command,
    options: OptionValues,
  ): Promise<void> {
    await new ConnectionCommandService(context).getProperties(
      options.dataPoolId,
  ...
  private async listConnections(
    ...
    await new ConnectionCommandService(context).listConnections(
      options.dataPoolId,
    );
  }

  private async updateConnectionProperty(
    context: Context,
    command: Command,
    options: OptionValues,
  ): Promise<void> {
    await new ConnectionCommandService(context).updateProperty(
      options.dataPoolId,
      options.connectionId,
      options.property,
      options.value,
