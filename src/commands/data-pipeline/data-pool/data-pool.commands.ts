...
export class DataPoolCommands {
  public register(context: Context, configurator: Configurator): void {
    ...
    exportCommand
      ...
      .option(
        "--outputToJsonFile",
        "Output the exported data pool to a JSON file",
      )
    ...
    importCommand
      .command("data-pools")
      .description(
        "Command to batch import multiple data pools with their objects and dependencies",
      )
      .requiredOption(
        "-f, --jsonFile <file>",
        "The file with the JSON data pool batch import request",
      )
      .option(
        "--outputToJsonFile",
        "Output the batch import result in a JSON file",
      )
    ...
    updateCommand
      .command("data-pool")
      .description(
        "Command to update a data pool using a data pool configuration file",
      )
  ...
  private async exportDataPool(
    context: Context,
    command: Command,
    options: OptionValues,
  ): Promise<void> {
    await new DataPoolCommandService(context).exportDataPool(
      options.id,
      options.outputToJsonFile,
    );
  }

  private async batchImportDataPools(
    context: Context,
    command: Command,
    options: OptionValues,
  ): Promise<void> {
    await new DataPoolCommandService(context).batchImportDataPools(
      options.jsonFile,
      options.outputToJsonFile,
    );
  }

  private async listDataPools(
    context: Context,
    command: Command,
    options: OptionValues,
  ): Promise<void> {
  ...
  private async updateDataPool(
    context: Context,
    command: Command,
    options: OptionValues,
  ): Promise<void> {
    await new DataPoolCommandService(context).updateDataPool(
      options.id,
      options.file,
    );
  }
