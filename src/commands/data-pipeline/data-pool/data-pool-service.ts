...
export class DataPoolService {
    ...
    public async batchImportDataPools(requestFilePath: string, outputToJsonFile: boolean): Promise<void> {
        ...
        const importReport = await this.dataPoolApi.executeDataPoolsBatchImport(requestFileContent);
    ...
    private async findAllPools(): Promise<DataPoolSlimTransport[]> {
        ...
        let tmpPage = await this.dataPoolApi.findAllPagedPools("100", page.toString());
        while (tmpPage.pageNumber < tmpPage.totalCount) {
            dataPools.push(...tmpPage.content);
            tmpPage = await this.dataPoolApi.findAllPagedPools("100", (++page).toString());
        }
        return dataPools;
    }
