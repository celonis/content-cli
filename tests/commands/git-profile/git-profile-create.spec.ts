...
describe("Git Profile - Create Profile", () => {
  ...
  it("Should create a Git profile with HTTPS authentication", async () => {
    ...
    expect(mockQuestionAsk).toHaveBeenNthCalledWith(
      1,
      "Name of the Git profile to create: ",
    );
    expect(mockQuestionAsk).toHaveBeenNthCalledWith(2, "Your Git username: ");
    expect(mockQuestionAsk).toHaveBeenNthCalledWith(
      3,
      "Your repository (format: repoOwner/repoName): ",
    );
    expect(mockQuestionAsk).toHaveBeenNthCalledWith(
      4,
      "Authentication type: HTTPS (1), SSH token (2): ",
    );
  ...
  it("Should not call makeDefaultProfile when setAsDefault is false", async () => {
    ...
    const writeFileCallsCount = (fs.writeFileSync as jest.Mock).mock.calls
      .length;
