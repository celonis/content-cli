...
describe("Node Dependencies", () => {
  ...
  it("Should list node dependencies and display in console", async () => {
    ...
    await new NodeDependencyService(testContext).listNodeDependencies(
      packageKey,
      nodeKey,
      version,
      false,
    ...
    expect(loggingTestTransport.logMessages[0].message.trim()).toBe(
      "Found 3 dependencies:",
    );
    expect(loggingTestTransport.logMessages[1].message).toContain(
      JSON.stringify(dependencies[0]),
    );
  ...
  it("Should list node dependencies and return as JSON", async () => {
    ...
    const dependenciesTransport = JSON.parse(
      mockWriteFileSync.mock.calls[0][1],
    ) as NodeDependencyTransport[];
  ...
  it("Should list single node dependency", async () => {
    ...
    expect(loggingTestTransport.logMessages[1].message).toContain(
      JSON.stringify(singleDependency[0]),
    );
  });
