...
describe("Config import", () => {
    ...
    it.each([true, false])("Should batch import package configs with overwrite %p", async (overwrite: boolean) => {
    ...
    it.each([true, false])(
        ...
        async (overwrite: boolean) => {
            ...
            jest.spyOn(GitService.prototype, "pullFromBranch").mockResolvedValue("mocked-pulled-git-path");
            mockReadFileSync(exportedPackagesZip.toBuffer());
            mockCreateReadStream(exportedPackagesZip.toBuffer());
            mockAxiosGet("https://myTeam.celonis.cloud/package-manager/api/packages", []);
    ...
    it.each([true, false])(
        ...
        async (overwrite: boolean) => {
            ...
            jest.spyOn(GitService.prototype, "pullFromBranch").mockResolvedValue(returnedGitPath);

            jest.spyOn(FileService.prototype, "zipDirectoryInBatchExportFormat").mockReturnValue("export_file.zip");
            const exportedPackagesZip = ConfigUtils.buildBatchExportZip(manifest, []);
    ...
    it("Should batch import configs & map space ID as specified in manifest file for Studio Packages", async () => {
        ...
        const exportedPackagesZip = ConfigUtils.buildBatchExportZipWithStudioManifest(manifest, studioManifest, [
            firstPackageZip,
        ]);
