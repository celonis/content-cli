...
describe("Config metadata export", () => {
  ...
  it("Should show on terminal if packages have unpublished changes", async () => {
    ...
    const expectedFirstString: string =
      "package-key-1 - Has Unpublished Changes: true";
    const expectedSecondString: string =
      "package-key-2 - Has Unpublished Changes: false";
    ...
    await new ConfigCommandService(testContext).batchExportPackagesMetadata(
      packageKeys,
      false,
    );
