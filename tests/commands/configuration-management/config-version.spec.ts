...
describe("Package Version get", () => {
  ...
  it("Should get package version metadata", async () => {
    ...
    const version = "1.2.3";
    mockAxiosGet(
      `https://myTeam.celonis.cloud/pacman/api/core/packages/${packageKey}/versions/${version}`,
      packageVersion,
    );

    await new PackageVersionCommandService(testContext).getPackageVersion(
      packageKey,
      version,
      false,
    );
    ...
    expect(loggingTestTransport.logMessages[0].message).toContain(
      `Package Key: ${packageVersion.packageKey}`,
    );
  ...
  it("Should get package version metadata and return as JSON", async () => {
    ...
    await new PackageVersionService(testContext).findPackageVersion(
      packageKey,
      version,
      true,
    );
    ...
    const packageVersionTransport = JSON.parse(
      mockWriteFileSync.mock.calls[0][1],
    ) as PackageVersionTransport;
  ...
  it("Should handle package version with empty publish message", async () => {
    ...
    mockAxiosGet(
      `https://myTeam.celonis.cloud/pacman/api/core/packages/${packageKey}/versions/${version}`,
      packageVersionWithEmptyMessage,
    );
