...
describe("List assignments", () => {
    it("Should list assignments for supported type and non-json response", async () => {
        const mockAssignmentValues = [{id: "id-1"}, {id: "id-2"}];
        ...
        expect(mockedAxiosInstance.get).toHaveBeenCalledWith(
            "https://myTeam.celonis.cloud/package-manager/api/compute-pools/pools-with-data-models",
            expect.anything()
        );
    });
    ...
    it("Should export assignments for supported type and json response", async () => {
        ...
        expect(mockWriteFileSync).toHaveBeenCalledWith(
            ...
            JSON.stringify(mockAssignmentValues),
            {encoding: "utf-8"}
        );
    });
    ...
    it("Should contain url params in the url", async () => {
        ...
        await new VariableCommandService(testContext).listAssignments(
            "CONNECTION",
            false,
        ...
        expect(mockedAxiosInstance.get).toHaveBeenCalledWith(
            "https://myTeam.celonis.cloud/process-automation-v2/api/connections?param1=value1&param2=value2",
            expect.anything()
        );
    });
