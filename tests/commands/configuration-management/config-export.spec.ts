...
describe("Config export", () => {
    ...
    it("Should export studio file for studio packageKeys", async () => {
        ...
        const studioManifest: StudioPackageManifest[] = parse(
            actualZip.getEntry(BatchExportImportConstants.STUDIO_FILE_NAME).getData().toString()
        );
    ...
    it("Should export to Github branch when branch is sent", async () => {
        ...
        expect(mockGitServicePushToBranch).toHaveBeenCalledWith(expect.any(String), branchName);
    ...
    it("Should export studio file for studio packageKeys and versions", async () => {
        const manifest: PackageManifestTransport[] = [];
        manifest.push(
            ConfigUtils.buildManifestForKeyAndFlavorAndVersion("key-1", BatchExportImportConstants.STUDIO, "1.0.1")
        );
        manifest.push(
            ConfigUtils.buildManifestForKeyAndFlavorAndVersion("key-2", BatchExportImportConstants.STUDIO, "1.0.4")
        );
    ...
    it("Should export variables file with connection variables fixed", async () => {
        ...
        const exportedVariablesFileContent: VariableManifestTransport[] = parse(
            actualZip.getEntry(BatchExportImportConstants.VARIABLES_FILE_NAME).getData().toString()
        );
    ...
    it("Should remove SCENARIO asset files of packages", async () => {
        ...
        expect(firstPackageExportedZip.getEntry("nodes/child-2.json").getData().toString()).toEqual(
            stringify(firstPackageTestChild)
        );
        ...
        expect(secondPackageExportedZip.getEntry("nodes/child-4.json").getData().toString()).toEqual(
            stringify(secondPackageTestChild)
        );
    });
    ...
    it("Should remove SCENARIO asset files of package keys that are sent with versions", async () => {
        const manifest: PackageManifestTransport[] = [];
        manifest.push(
            ConfigUtils.buildManifestForKeyAndFlavorAndVersion("key-1", BatchExportImportConstants.STUDIO, "1.0.2")
        );
        manifest.push(
            ConfigUtils.buildManifestForKeyAndFlavorAndVersion("key-2", BatchExportImportConstants.STUDIO, "1.0.3")
        );
    ...
    it("Should add appName to metadata for CONNECTION variables of package.json files when package keys are sent with versions", async () => {
        const manifest: PackageManifestTransport[] = [];
        manifest.push(
            ConfigUtils.buildManifestForKeyAndFlavorAndVersion("key-1", BatchExportImportConstants.STUDIO, "1.0.4")
        );
        manifest.push(
            ConfigUtils.buildManifestForKeyAndFlavorAndVersion("key-2", BatchExportImportConstants.STUDIO, "1.0.5")
        );
    ...
    it("Should export with SCENARIO nodes removed and CONNECTION variables fixed for package key with multiple underscores", async () => {
        const manifest: PackageManifestTransport[] = [];
        manifest.push(
            ConfigUtils.buildManifestForKeyAndFlavor("key_with_underscores_1", BatchExportImportConstants.STUDIO)
        );
        ...
        const firstPackageNode = ConfigUtils.buildPackageNode("key_with_underscores_1", {
            variables: firstPackageVariableDefinition,
        });
        const firstPackageScenarioChild = ConfigUtils.buildChildNode(
            "child-1-scenario",
