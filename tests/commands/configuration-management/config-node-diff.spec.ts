...
describe("Node diff", () => {
  ...
  it("Should diff two versions of a node", async () => {
    ...
    await new NodeDiffService(testContext).diff(
      packageKey,
      nodeKey,
      baseVersion,
      compareVersion,
    ...
    expect(loggingTestTransport.logMessages[6].message.trim()).toEqual(
      `Change Date: ${nodeDiff.changeDate}`,
    );
    expect(loggingTestTransport.logMessages[7].message.trim()).toEqual(
      `Updated By: ${nodeDiff.updatedBy}`,
    );
    expect(loggingTestTransport.logMessages[8].message.trim()).toEqual(
      `Change Type: ${nodeDiff.changeType}`,
    );
  ...
  it("Should diff two versions of a node and return as JSON", async () => {
    ...
    ) as NodeConfigurationDiffTransport;
  ...
  it.each(Object.keys(NodeConfigurationChangeType))(
    "Should diff node with %s change type",
    async changeType => {
  ...
  it("Should diff node with complex nested changes", async () => {
    ...
    const changesMessage = loggingTestTransport.logMessages.find(log =>
      log.message.includes("Changes:"),
    );
    ...
    expect(changesMessage.message.trim()).toEqual(
      `Changes: ${JSON.stringify(complexNodeDiff.changes)}`,
    );
  ...
  it("Should verify all logging output fields are present", async () => {
    ...
    const loggedFields = loggingTestTransport.logMessages.map(
      log => log.message,
    );
  ...
  it("Should format dates as ISO strings in output", async () => {
    ...
    const changeDateLog = loggingTestTransport.logMessages.find(log =>
      log.message.includes("Change Date:"),
    );
