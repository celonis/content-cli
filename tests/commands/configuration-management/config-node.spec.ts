...
describe("Node find", () => {
  ...
  it("Should find node without configuration", async () => {
    ...
    mockAxiosGet(
      `https://myTeam.celonis.cloud/pacman/api/core/staging/packages/${packageKey}/nodes/${nodeKey}?withConfiguration=false`,
      node,
    );

    await new NodeService(testContext).findNode(
      packageKey,
      nodeKey,
      false,
      null,
    ...
    expect(loggingTestTransport.logMessages[3].message).toContain(
      `Type: ${node.type}`,
    );
    expect(loggingTestTransport.logMessages[4].message).toContain(
      `Package Node Key: ${node.packageNodeKey}`,
    );
    expect(loggingTestTransport.logMessages[5].message).toContain(
      `Parent Node Key: ${node.parentNodeKey}`,
    );
    expect(loggingTestTransport.logMessages[6].message).toContain(
      `Created By: ${node.createdBy}`,
    );
    expect(loggingTestTransport.logMessages[7].message).toContain(
      `Updated By: ${node.updatedBy}`,
    );
    expect(loggingTestTransport.logMessages[8].message).toContain(
      `Creation Date: ${new Date(node.creationDate).toISOString()}`,
    );
    expect(loggingTestTransport.logMessages[9].message).toContain(
      `Change Date: ${new Date(node.changeDate).toISOString()}`,
    );
    expect(loggingTestTransport.logMessages[10].message).toContain(
      `Flavor: ${node.flavor}`,
    );
  });
  ...
  it("Should find versioned node without configuration", async () => {
    ...
    mockAxiosGet(
      `https://myTeam.celonis.cloud/pacman/api/core/packages/${packageKey}/nodes/${nodeKey}?version=${version}&withConfiguration=false`,
      node,
    );
  ...
  it("Should find node with configuration", async () => {
    ...
    mockAxiosGet(
      `https://myTeam.celonis.cloud/pacman/api/core/staging/packages/${packageKey}/nodes/${nodeKey}?withConfiguration=true`,
      nodeWithConfig,
    );
    ...
    expect(loggingTestTransport.logMessages[0].message).toContain(
      `ID: ${nodeWithConfig.id}`,
    );
    expect(loggingTestTransport.logMessages[10].message).toContain(
      `Configuration: ${JSON.stringify(nodeWithConfig.configuration, null, 2)}`,
    );
    expect(loggingTestTransport.logMessages[11].message).toContain(
      `Flavor: ${nodeWithConfig.flavor}`,
    );
  });
  ...
  it("Should find node and return as JSON", async () => {
    ...
    const nodeTransport = JSON.parse(
  ...
  it("Should find node with invalid configuration", async () => {
    ...
    const invalidConfigMessage =
      "Invalid JSON: Unexpected token at position 10";
    ...
    expect(loggingTestTransport.logMessages.length).toBe(12);
    expect(loggingTestTransport.logMessages[0].message).toContain(
      `ID: ${nodeWithInvalidConfig.id}`,
    );
    expect(loggingTestTransport.logMessages[10].message).toContain(
      `Invalid Configuration: ${invalidConfigMessage}`,
    );
    expect(loggingTestTransport.logMessages[11].message).toContain(
      `Flavor: ${nodeWithInvalidConfig.flavor}`,
    );
  });
