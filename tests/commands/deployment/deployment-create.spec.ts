...
describe("Deployment create", () => {
  ...
  it("Should return created deployment", async () => {
    mockAxiosPost(
      "https://myTeam.celonis.cloud/pacman/api/deployments",
      deployment,
    );

    await new DeploymentService(testContext).createDeployment(
      deployment.packageKey,
      deployment.packageVersion,
      deployment.deployableType,
  ...
  it("Should return created deployment as JSON", async () => {
    ...
    const deploymentTransport = JSON.parse(
