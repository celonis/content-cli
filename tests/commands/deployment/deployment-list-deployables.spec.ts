...
describe("Deployment list deployables", () => {
  ...
  it("Should list deployables", async () => {
    mockAxiosGet(
      "https://myTeam.celonis.cloud/pacman/api/deployments/deployables?",
      [appPackage, pigData],
    );
    ...
    expect(loggingTestTransport.logMessages[1].message).toContain(
      `Name: ${pigData.name}, Type: ${pigData.type}`,
    );
  });

  it("Should list deployables with flavor filter", async () => {
    mockAxiosGet(
      "https://myTeam.celonis.cloud/pacman/api/deployments/deployables?flavor=STUDIO",
      [appPackage],
    );
  ...
  it("Should list deployables as JSON", async () => {
    ...
    const deployableTransports = JSON.parse(
      mockWriteFileSync.mock.calls[0][1],
    ) as DeployableTransport[];
    expect(deployableTransports.length).toBe(2);

    const appPackageTransport = deployableTransports.filter(
      transport => transport.name === appPackage.name,
    )[0];
    const pigDataTransport = deployableTransports.filter(
      transport => transport.name === pigData.name,
    )[0];
