...
describe("Export action-flows", () => {
    ...
    it("Should call export API and return the zip response", async () => {
        ...
        const expectedZipFileName = loggingTestTransport.logMessages[0].message.split(
    ...
    it("Should call export API and attach the provided file to the zip response", async () => {
        ...
        expect(
            receivedZip.getEntries().filter(entry => entry.name === ActionFlowService.METADATA_FILE_NAME).length
        ).toBe(1);
