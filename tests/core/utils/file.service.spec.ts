...
describe("FileService", () => {
  ...
  const symLinkSourceTempDir = path.join(
    os.tmpdir(),
    "file-service-symlink-source",
  );
  ...
  describe("zipDirectoryInBatchExportFormat", () => {
    test("Should throw error if sourceDir is a symlink", () => {
      ...
      expect(() =>
        fileService.zipDirectoryInBatchExportFormat(symlinkDir),
      ).toThrow(FatalError);
    });
    ...
    test("Should skip symlinked folder and only include real folder", () => {
      ...
      const symLinkTargetFolder = path.join(
        symLinkSourceTempDir,
        "targetFolder",
      );
