...
describe("GitService", () => {
    ...
    describe("pullFromBranch", () => {
        ...
        it("should throw error when git profile is not configured", async () => {
            ...
            await expect(serviceWithoutProfile.pullFromBranch("main")).rejects.toThrow("No configured Git profile.");
        });
    });
    ...
    const assertSshClone = (branch: string, targetDir: string, repository: string) => {
        expect(mockGit.clone).toHaveBeenCalledWith(`git@github.com:${repository}.git`, targetDir, [
            "--branch",
            branch,
            "--single-branch",
        ]);
